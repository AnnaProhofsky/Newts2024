...
```
Looks like body length is normally distributed, and gape width is on the cusp. Let's zoom in on the gape width and take a look at a Q-Q plot
...
This means our only normally distributed data is body length with p-value = 0.9855. Let's move forward with the t-test there.

```{r t tests}
t.test(body_length ~ substrate, data = hatchling_data_filtered)
```
Significance! It appears that the ones wrapped in dead leaves were longer.
...
wilcox.test(interoccular_distance ~ substrate, data = hatchling_data_filtered)
